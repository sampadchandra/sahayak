...
export default function WorkerJoinPage() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const details = Object.fromEntries(formData.entries()) as Record<string, string>
      await saveWorkerProfile(formData)
  ...
  if (submitted) {
    const summary = [
      ['Full name', savedDetails.fullName], ['Phone', savedDetails.phone], ['Email', savedDetails.email],
      ['Primary skill', savedDetails.primarySkill], ['Experience', savedDetails.experience],
      ['Service area', savedDetails.serviceArea], ['Languages', savedDetails.languages],
      ['Availability', savedDetails.availability], ['Other skills', savedDetails.otherSkills],
    ].filter(([, value]) => value)

    return <main className="min-h-screen bg-background px-5 py-12 text-foreground"><div className="mx-auto max-w-2xl"><div className="text-center"><span className="mx-auto grid size-16 place-items-center rounded-3xl bg-primary text-primary-foreground"><Check className="size-8" /></span><h1 className="mt-6 font-serif text-4xl font-bold">Your profile is ready for review.</h1><p className="mx-auto mt-4 max-w-lg leading-7 text-muted-foreground">Your details have been saved in SAHAYAK and will be used for suitable worker matching.</p></div><section className="mt-10 rounded-[2rem] border border-border bg-card p-6 shadow-sm sm:p-8"><div className="flex items-center justify-between border-b border-border pb-5"><div><p className="eyebrow">Saved profile</p><h2 className="mt-1 font-serif text-2xl font-bold">Review your details</h2></div><span className="rounded-full bg-secondary px-3 py-1 text-xs font-semibold text-primary">Saved</span></div><dl className="mt-6 grid gap-4 sm:grid-cols-2">{summary.map(([label, value]) => <div key={label} className="rounded-xl bg-background p-4"><dt className="text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">{label}</dt><dd className="mt-1 text-sm font-medium">{value}</dd></div>)}</dl></section><div className="text-center"><Link href="/" className="mt-8 inline-flex rounded-full bg-primary px-5 py-3 font-semibold text-primary-foreground">Back to SAHAYAK</Link></div></div></main>
  }
