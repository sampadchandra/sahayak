...
export async function updateBookingStatus(bookingId: string, nextStatus: string) {
  ...
  const [worker] = await db.select().from(workerProfile).where(eq(workerProfile.userId, user.id)).limit(1)
  const isCustomer = row.userId === user.id
  const isWorker = worker?.id === row.workerId
  ...
  await db.update(booking).set({ status: nextStatus, updatedAt: new Date() }).where(and(eq(booking.id, bookingId), isCustomer ? eq(booking.userId, user.id) : eq(booking.workerId, worker?.id ?? '')))
  await db.insert(alert).values({ id: randomUUID(), userId: isCustomer ? (worker?.userId ?? row.userId) : row.userId, bookingId, type: `BOOKING_${nextStatus}`, title: `Booking ${nextStatus.toLowerCase().replace('_', ' ')}`, body: `Your booking status is now ${nextStatus.toLowerCase().replace('_', ' ')}.` })
