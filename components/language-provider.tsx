...
const copy = {
  English: {
    'Find help': 'Find help', 'How it works': 'How it works', 'Built for everyday India': 'Built for everyday India', 'Start here': 'Start here', 'Change': 'Change', 'For workers': 'For workers', 'Our impact': 'Our impact', 'Sign in': 'Sign in', 'Get started': 'Get started',
  ...
  'हिन्दी': {
    ...
    'Built for everyday India': 'हर दिन के भारत के लिए बनाया गया', 'Start here': 'यहाँ से शुरू करें', 'Change': 'बदलें', 'Trusted help.': 'भरोसेमंद मदद।', 'Fair work.': 'न्यायपूर्ण काम।', 'Better lives.': 'बेहतर जीवन।', 'Find trusted help': 'भरोसेमंद मदद खोजें', 'See how it works': 'देखें कैसे काम करता है', 'Verified workers': 'सत्यापित वर्कर्स', 'Safety first': 'सुरक्षा पहले', 'Fair earnings': 'न्यायपूर्ण कमाई',
  ...
  'বাংলা': {
    ...
    'Built for everyday India': 'প্রতিদিনের ভারতের জন্য তৈরি', 'Start here': 'এখান থেকে শুরু করুন', 'Change': 'পরিবর্তন করুন', 'Trusted help.': 'বিশ্বস্ত সাহায্য।', 'Fair work.': 'ন্যায্য কাজ।', 'Better lives.': 'উন্নত জীবন।', 'Find trusted help': 'বিশ্বস্ত সাহায্য খুঁজুন', 'See how it works': 'কীভাবে কাজ করে দেখুন', 'Verified workers': 'যাচাই করা কর্মী', 'Safety first': 'নিরাপত্তা আগে', 'Fair earnings': 'ন্যায্য উপার্জন',
...
export function LanguageProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => { const raw = document.cookie.match(/sahayak-language=([^;]+)/)?.[1]; const saved = raw ? decodeURIComponent(raw) as Language : undefined; if (saved && saved in copy) { setLanguageState(saved); document.documentElement.lang = saved === 'हिन्दी' ? 'hi' : saved === 'বাংলা' ? 'bn' : 'en' } }, [])
  ...
  useEffect(() => {
    const translate = () => {
      ...
      textNodes.forEach((node) => {
        ...
        if (key && node.parentElement?.tagName !== 'SCRIPT' && node.parentElement?.tagName !== 'STYLE') {
          const translated = dictionary[key] ?? (language === 'English' ? key : key)
          const start = source.indexOf(key)
          const nextText = `${source.slice(0, start)}${translated}${source.slice(start + key.length)}`
          if (node.textContent !== nextText) node.textContent = nextText
        }
      })
      document.querySelectorAll('body *').forEach((element) => {
        if (!originalElementAttributes.has(element)) originalElementAttributes.set(element, {})
        const attributes = originalElementAttributes.get(element)!
        ;['placeholder', 'aria-label', 'title'].forEach((attribute) => {
          if (!(attribute in attributes)) attributes[attribute] = element.getAttribute(attribute)
          const source = attributes[attribute]
          if (source) {
            const nextValue = dictionary[source] ?? source
